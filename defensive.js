...
function randomFruitPosition() {
    while (fruits.length < maxFruits) {
        ...
        if (!hasSpecialFruit && Math.random() < 0.1) { // Adjust the probability
            ...
            // Generate a special fruit far away from both snakes
            const minDistanceThreshold = 5; // Adjust this value for desired minimum distance
            ...
            for (let attempt = 0; attempt < maxAttempts; attempt++) {
                ...
                // Check if the special fruit position is far enough from both snakes
                const distanceToPlayerSnake = Math.abs(newFruit.x - playerSnake.x) + Math.abs(newFruit.y - playerSnake.y);
                const distanceToAISnake = Math.abs(newFruit.x - aiSnake.x) + Math.abs(newFruit.y - aiSnake.y);

                if (distanceToPlayerSnake >= minDistanceThreshold && distanceToAISnake >= minDistanceThreshold) {
                    break;
                }
            }
...
function drawFruits() {
    fruits.forEach(fruit => {
        ctx.fillStyle = fruit.isSpecial ? 'orange' : 'green'; // Adjust color for special fruit
...
function moveAISnake() {
    ...
    if (closestFruit) {
        ...
        if (aiPath.length > 1) {
            ...
            // Check if the next step collides with the player snake's body
            const collidesWithPlayerSnake = playerSnake.tail.some(segment => (
                segment.x === nextStep.x && segment.y === nextStep.y
            ));

            if (!collidesWithPlayerSnake) {
                const dx = nextStep.x - aiSnake.x;
                const dy = nextStep.y - aiSnake.y;

                // Prefer horizontal movement if possible
            ...
            // No valid path, choose a random safe direction
            const possibleDirections = [];

            // Check if moving up is safe
            const upPosition = { x: aiSnake.x, y: (aiSnake.y - 1 + gridSize) % gridSize };
            if (!avoidPositions.some(pos => pos.x === upPosition.x && pos.y === upPosition.y)) {
                possibleDirections.push('up');
            }

            // Check if moving down is safe
            const downPosition = { x: aiSnake.x, y: (aiSnake.y + 1) % gridSize };
            if (!avoidPositions.some(pos => pos.x === downPosition.x && pos.y === downPosition.y)) {
                possibleDirections.push('down');
            }

            // Check if moving left is safe
            const leftPosition = { x: (aiSnake.x - 1 + gridSize) % gridSize, y: aiSnake.y };
            if (!avoidPositions.some(pos => pos.x === leftPosition.x && pos.y === leftPosition.y)) {
                possibleDirections.push('left');
            }

            // Check if moving right is safe
            const rightPosition = { x: (aiSnake.x + 1) % gridSize, y: aiSnake.y };
            if (!avoidPositions.some(pos => pos.x === rightPosition.x && pos.y === rightPosition.y)) {
                possibleDirections.push('right');
            }

            // Choose a random direction from the safe ones
            if (possibleDirections.length > 0) {
                const randomDirection = possibleDirections[Math.floor(Math.random() * possibleDirections.length)];
                aiSnake.direction = randomDirection;
            }
        }
    }
}
